...
class Subject:
    ...
    @json_response
    async def search(self, keyword: str, s_type: int = 2, response_group="small", start: int = 0, max_results: int = 8):
        """
        获取系统信息
        :return:
        """
        return await self.client.get(f"search/subject/{keyword}", params={
            "type": s_type, "responseGroup": response_group, "start": start, "max_results": max_results})
